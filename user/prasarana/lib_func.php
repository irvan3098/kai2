...
HAVING j.kategori = 'prasarana'");		
		...
		?>
                    <li>
                        <a href="detail_data.php?id=<?php echo $data["katsar"];?>&judul=<?php echo $data["katsar"];?>""><?php echo $data["katsar"];?></a>
                    </li>
       <?php
			}
			?>
...
<?php
	function sarfas()
	{
		...
		where kategori='prasarana' and year(tanggal) = '$thn_'");
		if(!$res){
			die("Query error! : ".mysqli_error($link));
		}
			$i=1;
...
?>
				<tr class="odd gradeX">
                    ...
                    <td>
                        <a href="#" onClick="window.open('http://<?php url();?>user/prasarana/detail_cetak.php?id=<?php echo $id; ?>&judul=<?php echo $judul;?>','popuppage','width=500,toolbar=0,resizable=0,scrollbars=no,height=400,top=100,left=100');" >
                        Download
                        </a>
...
function ditail_cetak()
{
		...
		WHERE judul.kategori='prasarana' AND YEAR(judul.tanggal)=YEAR(NOW()) and judul.id_judul='$id' or sar.kategori='$id'");		
		if(!$res){
			die("Query error! : ".mysqli_error($link));
		}
		$i=1;
...
function ditail_data()
{
		...
		from sarana_dan_fasilitas sar
		join judul on (judul.id_judul = sar.id_judul) 
		where judul.kategori='prasarana' and YEAR(judul.tanggal)=YEAR(NOW()) and judul.id_judul='$id' or judul.kategori='$id'");		
		if(!$res){
			die("Query error! : ".mysqli_error($link));
		}
		$i=1;
...
<?php
	function detail_hascar()
	{
		...
		from sarana_dan_fasilitas sar
		join judul on (judul.id_judul = sar.id_judul) 
		where judul.kategori='prasarana' and YEAR(judul.tanggal)=YEAR(NOW()) and sar.nama_brg like '%$cari%' or sar.no_sap like '%$cari%'");		
		if(!$res){
			die("Query error! : ".mysqli_error($link));
		}
		$i=1;
...
function cetak_hascar()
{
		...
		FROM sarana_dan_fasilitas sar
		JOIN judul ON (judul.id_judul = sar.id_judul) 
		WHERE judul.kategori='prasarana' AND YEAR(judul.tanggal)=YEAR(NOW()) and sar.nama_brg like '%$id%'");		
		if(!$res){
			die("Query error! : ".mysqli_error($link));
		}
		$i=1;
