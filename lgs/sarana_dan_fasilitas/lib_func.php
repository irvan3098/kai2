...
<?php
	function nav()
	{
		...
		$link=koneksi();
		$res=mysqli_query($link,"select * from judul where kategori='sarana dan fasilitas'");		
		if(!$res){
...
<?php
	function data_hasilcari()
	{
		...
		$link=koneksi();
		$cari=$_GET["cari"];
		$res=mysqli_query($link,"SELECT judul.judul, judul.kategori,judul.tanggal,judul.file, judul.no_kontrak,sar.id_sarfas, sar.no_sap, sar.kategori, sar.nama_brg, sar.spesifikasi, sar.satuan, sar.mata_uang, sar.harga_satuan, sar.vendor
		from sarana_dan_fasilitas sar
		join judul on (judul.id_judul = sar.id_judul) 
		where judul.kategori='sarana dan fasilitas' and sar.nama_brg like '%$cari%' or sar.no_sap like '%$cari%'");		
		if(!$res){
			die("Query error! : ".mysqli_error($link));
		}
		$i=1;
	...
	function tambah_judul()
	{
		...
		if(isset($_POST["tambahjudul"]))
		{
			...
			else
			{
				...
				{
					$sql="insert into judul values(NULL,'sarana dan fasilitas','$judul','$no_kontrak','$tgl',NULL)"; 
				...
				}else
				{
					...
					$sql="insert into judul values(NULL,'sarana dan fasilitas','$judul','$no_kontrak','$tgl','$file')"; 
...
<?php
	function data_judul()
	{
		error_reporting(E_ALL ^ (E_NOTICE | E_WARNING));
		$link=koneksi();
		$res=mysqli_query($link,"SELECT id_judul,judul,no_kontrak,tanggal,REPLACE(judul,' ','') AS kode, file from judul where kategori='sarana dan fasilitas'");		
		if(!$res){
			die("Query error! : ".mysqli_error($link));
		}
		$j=1;
